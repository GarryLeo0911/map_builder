...
static rclcpp::Logger LOGGER_HOLE = rclcpp::get_logger("map_builder::hole_filling");
...
int fill_hole(std::string mesh_path, double max_perimeter, std::string base_path) {
    ...
    if (!input || !(input >> poly) || poly.empty()) {
        RCLCPP_ERROR_STREAM(LOGGER_HOLE, "Not a valid off file (fill_hole): " << filename);
        return 0;
    }
    ...
    RCLCPP_DEBUG_STREAM(LOGGER_HOLE, nb_holes << " holes have been filled");
