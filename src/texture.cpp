...
{	
	...
	rclcpp::Logger logger = rclcpp::get_logger("map_builder");
	RCLCPP_DEBUG_STREAM(logger, "Number of faces of the mesh: " << nfaces);
	RCLCPP_DEBUG_STREAM(logger, "Created a grid of " << l << " by " << l << " for per-face uv parametrization.");
	RCLCPP_DEBUG_STREAM(logger, "Grid square size (pixels): " << t*texture_size);
	...
	for(int i=0; i< l;i++)
	{
		...
		float percentage = (i/(float)(l-1)) * 100.0;
		if(fmod(percentage, 10.0f) < 0.1) {
			RCLCPP_DEBUG(logger, "Baking texture... (%.2f%% done)", percentage);
		}
	}

	RCLCPP_DEBUG_STREAM(logger, "Writing texture: " << output_path+"_texture.png");
