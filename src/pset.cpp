...
static rclcpp::Logger LOGGER_PSET = rclcpp::get_logger("map_builder::pset");
...
void Pset::read_ply(std::string path) {
    ...
    RCLCPP_DEBUG_STREAM(LOGGER_PSET, "Reading PLY file: " << path);
    ...
    if (has_normals)
        RCLCPP_DEBUG_STREAM(LOGGER_PSET, "PLY has normals.");
    if (has_alpha)
        RCLCPP_DEBUG_STREAM(LOGGER_PSET, "PLY has alpha channel");
...
void Pset::read_csv(std::string path) {
    //FT xc = (FT)0., yc = (FT)0., zc = (FT)0.,
    RCLCPP_DEBUG_STREAM(LOGGER_PSET, "Reading CSV file: " << path);
    ...
    if (has_color) {
        RCLCPP_DEBUG_STREAM(LOGGER_PSET, "Reading colored points.");
        ...
        RCLCPP_DEBUG_STREAM(LOGGER_PSET, "Reading points without color.");
        myfile >> x >> v >> y >> v >> z;
...
void Pset::sample_points_cgal(double param, int method) {
    ...
    RCLCPP_DEBUG_STREAM(LOGGER_PSET, "Sample points:" << param);
