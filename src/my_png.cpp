#include "map_builder/my_png.hpp"
...
unsigned char &my_png::at(int r, int c, int ch) {
    if (r < height && c < width && r >= 0 && c >= 0)
        ...
        RCLCPP_DEBUG_STREAM(LOGGER, "Error: image coords out of bounds");
        RCLCPP_DEBUG_STREAM(LOGGER, "(" << r << ", " << c << ")");
...
void my_png::write(string filename) {
    ...
    if (setjmp(png_jmpbuf(png_ptr)))
        RCLCPP_DEBUG_STREAM(LOGGER, "error in init_io");
