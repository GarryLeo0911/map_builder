...
std::list<PointVectorPair> grab_normals(std::vector<Point> &pts, std::vector<Vector> &norms) {
    ...
    RCLCPP_DEBUG_STREAM(LOGGER, "Using provided normals...");
...
//        std::cout<<std::setprecision(12)<<"x y z: "<<(*it).first<<"  nx ny nz: "<<(*it).second<<std::endl;
    ...
    RCLCPP_DEBUG_STREAM(LOGGER, "Num points:" << points.size());
    ...
    RCLCPP_DEBUG_STREAM(LOGGER, "Normals estimated.");
...
{
    RCLCPP_DEBUG_STREAM(LOGGER, "Calls mst_orient_normals()\n");
    ...
    RCLCPP_DEBUG_STREAM(LOGGER, mb_memory << " Mb allocated\n");
    RCLCPP_DEBUG_STREAM(LOGGER, "  Create Index_property_map\n");
...
*/
    ...
    RCLCPP_DEBUG_STREAM(LOGGER, "Num vertices riemannian graph:" << num_input_points);
    ...
    RCLCPP_DEBUG_STREAM(LOGGER, "Num vertices mst graph:" << num_input_points2);

    memory = CGAL::Memory_sizer().virtual_size();
    mb_memory = memory >> 20;
    RCLCPP_DEBUG_STREAM(LOGGER, mb_memory << "Mb allocated");
    RCLCPP_DEBUG_STREAM(LOGGER, "  Calls boost::breadth_first_search()");
    ...
    RCLCPP_DEBUG_STREAM(LOGGER, "  => " << unoriented_points.size() << " normals are unoriented\n");

    memory = CGAL::Memory_sizer().virtual_size();
    
    mb_memory = memory >> 20;
    RCLCPP_DEBUG_STREAM(LOGGER, mb_memory << " Mb allocated\n");
    RCLCPP_DEBUG_STREAM(LOGGER, "End of mst_orient_normals()\n");
...
std::list<PointVectorPair> register_normals(std::vector<Point> sampled_points, std::list<PointVectorPair> original) {
    ...
    RCLCPP_DEBUG_STREAM(LOGGER, "Normals registered and refined...");
    return refined;
}
...
//    FT sm_distance = 0.5;//0.375; // Surface Approximation error w.r.t. point set average spacing. //0.5
    ...
    RCLCPP_DEBUG_STREAM(LOGGER, "Surface recon started...");
    ...
    if (!function.compute_implicit_function())
        RCLCPP_DEBUG_STREAM(LOGGER, "Error in computing implicit function");
...
//                                                        sm_distance); // * average_spacing); // Approximation error
    ...
    RCLCPP_DEBUG_STREAM(LOGGER, "3D Delaunay triangulation...");
    ...
    if (tr.number_of_vertices() == 0)
        RCLCPP_DEBUG_STREAM(LOGGER, "Warning: mesh has 0 vertices");
...
void trim_mesh(Mesh m, Tree &tree, double threshold, std::string base_path) {
    Distance tr_dist;
    RCLCPP_DEBUG_STREAM(LOGGER, "Threshold for trim: " << threshold);
...
int remove_borders(std::string mesh_path, int iterations, std::string base_path) {
    ...
    if(iterations <= 0){
        RCLCPP_ERROR_STREAM(LOGGER, "Not removing borders (iterations is <= 0): " << iterations);
        return 0;
    }
    ...
    Mesh m;
    if (!input || !(input >> m)) {
        RCLCPP_WARN_STREAM(LOGGER, "Not a valid off file (remove_borders): " << filename);
        return 0;
    }
    ...
    //RCLCPP_DEBUG_STREAM(LOGGER, borderVertexID.size() << " size of the longest border at iteration " << iter_idx);
    ...
    // RCLCPP_DEBUG_STREAM(LOGGER, removed_faces << " border faces have been removed at iteration " << iter_idx);
        
        m.collect_garbage();
    }

    RCLCPP_DEBUG_STREAM(LOGGER, num_faces(m) << "/" << faces_before_removal << " faces remaining (" << removed_faces << " border faces removed)");
