#!/usr/bin/env python3
"""ROS2 test client that subscribes to a pointcloud topic and requests
point-normal estimation service (p-normals) from the recon_surface service.
"""
import time
import traceback
import struct
...
from map_builder.srv import NormalsFromPointCloud2
...
class TestPNormalsService(Node):
    def __init__(self):
        super().__init__('test_p_normals_service')
        self.create_subscription(sensor_msgs.PointCloud2, '/test_point_cloud', self.pointcloud2_callback, 10)
        self.client = self.create_client(NormalsFromPointCloud2, '/pnormal_from_pointclouds')
    ...
    def pointcloud2_callback(self, msg):
        ...
        try:
            if not self.client.wait_for_service(timeout_sec=3.0):
                self.get_logger().error('p-normals service not available')
                return
            ...
            time1 = time.time()
            req = NormalsFromPointCloud2.Request()
